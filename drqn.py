...
def GT_transform(old_gt,row,col):
	newGT = np.zeros((old_gt.shape[0],4))
    x = gt[:,0::2];    
    y = gt[:,1::2];    
    newGT[:,0] = np.amin(x,axis=1)
    newGT[:,1] = np.amin(y,axis=1)
    newGT[:,2] = np.amax(x,axis=1) - np.amin(x,axis=1)
    newGT[:,3] = np.amax(y,axis=1) - np.amin(y,axis=1)
    newGT[:,0] = (newGT[:,0]/col)*224
    newGT[:,1] = (newGT[:,1]/row)*224
    return newGT
